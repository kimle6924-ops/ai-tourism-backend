...
public class GeminiTestController : ControllerBase
{
    ...
    private static string BuildAskApiSystemPrompt()
    {
        return $"""
            ...
            Dưới đây là toàn bộ tài liệu API endpoints của dự án, bao gồm validation rules, business logic và error codes:

            {ApiDocSections.General()}

            {ApiDocSections.Auth()}

            {ApiDocSections.User()}

            {ApiDocSections.Admin()}

            {ApiDocSections.AdministrativeUnits()}

            {ApiDocSections.Categories()}

            {ApiDocSections.Places()}

            {ApiDocSections.Events()}

            {ApiDocSections.Moderation()}

            {ApiDocSections.Media()}

            {ApiDocSections.Reviews()}

            {ApiDocSections.Leaderboard()}
